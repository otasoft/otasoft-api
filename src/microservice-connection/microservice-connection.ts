...
export const connectMicroservice = (microserviceName: string):ClientsProviderAsyncOptions => {
    ...
    const microserviceOptions: ClientsProviderAsyncOptions = {
        ...
        useFactory: (configService: ConfigService) => ({
            ...
            options: {
                urls: [
                `amqp://${configService.get('RABBITMQ_DEFAULT_USER')}:${configService.get('RABBITMQ_DEFAULT_PASS')}@${process.env.RABBITMQ_NODENAME}:${configService.get('RABBITMQ_FIRST_HOST_PORT')}/${configService.get('RABBITMQ_DEFAULT_VHOST')}` 
                ],
